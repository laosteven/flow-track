...
class AdvancedMetricsCard extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildMetricRow(
              ...
              value: '${metrics.entryAngle.toStringAsFixed(0)}° → ${metrics.exitAngle.toStringAsFixed(0)}°',
              color: Colors.purple,
            ),
            const SizedBox(height: 12),
            _buildMetricRow(
            ...
            const SizedBox(height: 12),
            _buildPhaseIndicator(context, metrics.strokePhase, metrics.phaseString),
          ],
        ),
      ),
    );
  ...
  Widget _buildPhaseIndicator(BuildContext context, int phase, String phaseString) {
    return Container(
  ...
  String _getSmoothnessText(double smoothness) {
    if (smoothness > 0.8) return '${(smoothness * 100).toStringAsFixed(0)}% Excellent';
    if (smoothness > 0.6) return '${(smoothness * 100).toStringAsFixed(0)}% Good';
    if (smoothness > 0.4) return '${(smoothness * 100).toStringAsFixed(0)}% Fair';
  ...
  IconData _getPhaseIcon(int phase) {
    switch (phase) {
      case 0: return Icons.stop_circle;
      case 1: return Icons.water_drop;
      case 2: return Icons.flash_on;
      case 3: return Icons.eject;
      case 4: return Icons.replay;
